...
class SafeCloudUploader(
    ...
    suspend fun upload(projectId: String, plan: FloorPlan): UploadResult = withContext(Dispatchers.IO) {
        ...
        observed?.let { body.put("expected_revision", it.revision) }

        val text = execute(projectId, body)
        val root = JSONObject(text)
        val revision = root.optInt("revision", plan.revision).coerceAtLeast(1)
    ...
    private suspend fun execute(projectId: String, body: JSONObject): String {
        ...
        if (code == 409) {
            val current = runCatching {
                ...
                detail?.optInt("current_revision", -1)?.takeIf { it > 0 }
            }.getOrNull()
