...
private fun ProductionHome(nav:NavHostController,plan:FloorPlan?,count:Int){
    Surface(Modifier.fillMaxSize(),color=Color(0xFFF7F4EE)){
        Column(Modifier.fillMaxSize().statusBarsPadding().navigationBarsPadding().padding(20.dp)){
            ...
            Text("$count مشروع • Saudi-first 2D/PBR 3D/First-person/BIM 4D • Geometry V3 • Deep Parser",color=Color.Gray,fontSize=9.5.sp)
            ...
            if(plan!=null){
                ...
                Spacer(Modifier.height(6.dp));Row(horizontalArrangement=Arrangement.spacedBy(7.dp)){OutlinedButton(onClick={nav.navigate("walkthrough")},modifier=Modifier.weight(1f)){Icon(Icons.Rounded.DirectionsWalk,null);Spacer(Modifier.width(4.dp));Text("جولة 3D")};OutlinedButton(onClick={nav.navigate("4d")},modifier=Modifier.weight(1f)){Icon(Icons.Rounded.Schedule,null);Spacer(Modifier.width(4.dp));Text("4D / BIM")}}
