...
/**
 * Adds presentation-grade architectural meshes on top of the canonical Semantic3D scene.
 * Geometry still comes from the same plan: no second floorplan and no generative 3D guessing.
 * Door/window leaves use confirmed vertical profiles when available; otherwise they remain preview-only.
 */
object Architectural3DEnhancementEngine {
        ...
        floorSources(plan).forEachIndexed { index, floor ->
            ...
            floor.openings.forEach { opening ->
                ...
                thinPanel(
                    ...
                    width = node.width,
                    thickness = if (metricReady) 0.035 else 0.18,
                    sx = sx,
                    sy = sy
                )?.let(extra::add)
            }
            ...
            if (footprint.size >= 3) {
                ...
                extra += prism(
                    ...
                    floorId = floor.id,
                    sourceId = floor.id,
                    polygon = footprint.map { it.x * sx to it.y * sy },
                    zMin = z,
                    zMax = z + roofThickness
                )
            } else {
                warnings += "${floor.name}: لم أضف سقف 3D لأن حدود الدور غير كافية."
            }
        }
    ...
    private fun floorSources(plan: FloorPlan): List<FloorSource> = if (plan.floors.isNotEmpty()) {
        plan.floors.sortedBy { it.index }.map {
            FloorSource(it.id, it.name, it.elevationM, it.clearHeightM, it.footprint, it.walls, it.openings)
        }
    } else {
        listOf(FloorSource(plan.activeFloorId ?: "ground", "الدور الأرضي", 0.0, null, plan.footprint, plan.walls, plan.openings))
    }
    ...
    private fun thinPanel(
        ...
        val ax = wall.start.x * sx
        val ay = wall.start.y * sy
        val bx = wall.end.x * sx
        val by = wall.end.y * sy
        val dx = bx - ax
        val dy = by - ay
        val len = hypot(dx, dy)
        if (len < 1e-6 || width <= 0.0 || zMax <= zMin) return null
        val ux = dx / len
        val uy = dy / len
        val px = -uy
        val py = ux
        ...
        val a = centerX - ux * halfW to centerY - uy * halfW
        val b = centerX + ux * halfW to centerY + uy * halfW
        val v = listOf(
            Semantic3DEngine.Vec3(a.first + px * halfT, a.second + py * halfT, zMin),
            Semantic3DEngine.Vec3(b.first + px * halfT, b.second + py * halfT, zMin),
            Semantic3DEngine.Vec3(b.first - px * halfT, b.second - py * halfT, zMin),
            Semantic3DEngine.Vec3(a.first - px * halfT, a.second - py * halfT, zMin),
            Semantic3DEngine.Vec3(a.first + px * halfT, a.second + py * halfT, zMax),
            Semantic3DEngine.Vec3(b.first + px * halfT, b.second + py * halfT, zMax),
            Semantic3DEngine.Vec3(b.first - px * halfT, b.second - py * halfT, zMax),
            Semantic3DEngine.Vec3(a.first - px * halfT, a.second - py * halfT, zMax)
        )
        return Semantic3DEngine.Mesh(
            id, kind, name, floorId, sourceId, v,
            listOf(
    ...
    private fun prism(
        ...
        val n = polygon.size
        val vertices = polygon.map { Semantic3DEngine.Vec3(it.first, it.second, zMin) } +
            polygon.map { Semantic3DEngine.Vec3(it.first, it.second, zMax) }
        ...
        repeat(n) { i ->
            val j = (i + 1) % n
            faces += Semantic3DEngine.Face(listOf(i, j, n + j, n + i))
        }
    ...
    private fun deriveFootprint(walls: List<Wall>): List<PlanPoint> {
        val pts = walls.flatMap { listOf(it.start, it.end) }
