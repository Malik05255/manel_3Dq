...
import com.manzili.hai.engine.GeometryV3Engine
...
class EngineRegressionTest {
    ...
    @Test fun geometryV3AttachesWallTopology() {
        val a = room("a", 0f, 0f, 50f, 100f)
        val b = room("b", 50f, 0f, 50f, 100f)
        ...
        assertEquals(setOf("a","b"), result.plan.walls.first().adjacentRoomIds.toSet())
    }
    ...
    @Test fun geometryV3RejectsElementOutsideBoundary() {
        ...
        assertTrue(result.errors.any { it.contains("خرج") })
    }

    @Test fun dxfContainsVectorLayersAndMeters() {
        val plan = FloorPlan(
            widthM = 20.0, heightM = 25.0, scaleConfidence = 100,
            rooms = listOf(room("r1", 0f,0f,50f,50f)),
            walls = listOf(Wall("w1", PlanPoint(0f,0f), PlanPoint(100f,0f), confidence = 100)),
            footprint = boundary()
        )
        val dxf = DxfPlanExporter.render(plan)
        assertTrue(dxf.contains("MANZILI_HAI_METERS"))
        assertTrue(dxf.contains("F0_WALLS"))
    ...
    @Test fun dimensionParserDoesNotTreatBareNumbersAsMeasurements() {
        ...
        assertFalse(dims.any { it.valueM == 12.0 })
        assertTrue(dims.any { kotlin.math.abs(it.valueM - 3.5) < .001 })
        assertTrue(dims.any { kotlin.math.abs(it.valueM - 4.0) < .001 })
    }

    @Test fun parserFusionRaisesConfidenceOnMatchingRasterWall() {
        val base = FloorPlan(
            rooms = listOf(room("r1", 0f,0f,100f,100f)),
            walls = listOf(Wall("w1", PlanPoint(0f,0f), PlanPoint(100f,0f), confidence = 55)),
            footprint = boundary()
        )
        val raster = Wall("rw", PlanPoint(0f,.4f), PlanPoint(100f,.4f), confidence = 70)
        val result = FloorplanParserEngine.refine(base, listOf(raster)).plan
    ...
    @Test fun storageSchemaFiveRoundTripsV3Fields() {
        ...
        val input = FloorPlan(
            title = "roundtrip", widthM = 20.0, heightM = 25.0, scaleConfidence = 100,
            walls = listOf(Wall("w", PlanPoint(0f,0f), PlanPoint(100f,0f), adjacentRoomIds = listOf("r"))),
            dimensions = listOf(PlanDimension("d", "عرض", 3.5, start = PlanPoint(1f,2f), end = PlanPoint(5f,2f), pageIndex = 2)),
            elements = listOf(element), saudiRulesEnabled = true
        )
        val json = PlanStorageCodec.encode(input).toString()
        assertEquals(5, JSONObject(json).getInt("schemaVersion"))
        val output = PlanStorageCodec.decode(JSONObject(json))
        assertEquals(2, output.dimensions.single().pageIndex)
        assertEquals("stair", output.elements.single().type)
        assertEquals(listOf("r"), output.walls.single().adjacentRoomIds)
        assertTrue(output.saudiRulesEnabled)
    }

    @Test fun optimizerReturnsDistinctFeasibleSolutions() {
        val program = NewBuildSolver.Program(
            city = "الرياض", plotWidthM = 20.0, plotDepthM = 25.0, floorCount = 1,
            bedrooms = 3, guestEntranceIndependent = true, privacyPriority = 90,
            circulationPriority = 85, daylightPriority = 80
        )
        val candidates = NewBuildOptimizer.generate(program)
        assertEquals(3, candidates.size)
        assertEquals(3, candidates.map { it.plan.rooms.joinToString("|") { r -> "${r.id}:${r.x}:${r.y}:${r.width}:${r.height}" } }.distinct().size)
        assertTrue(candidates.all { GeometryV3Engine.inspect(it.plan).valid })
    }
